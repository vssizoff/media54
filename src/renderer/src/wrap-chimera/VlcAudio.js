...
class VlcAudio {
     ...
     */
    get channel() {
        return enums.Channel[this._audio.channel];
    }
     ...
     */
    set channel(channel) {
        if (!enums.Channel.includes(channel))
            throw new Error("Channel should be one of " + enums.Channel.toString())
        this._audio.channel = enums.Channel.indexOf(channel);
    }
