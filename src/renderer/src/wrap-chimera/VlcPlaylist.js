...
class VlcPlaylist {
     ...
     */
    get mode() {
        return enums.PlaylistMode[this._playlist.mode];
    }
     ...
     */
    set mode(mode) {
        if (!enums.PlaylistMode.includes(mode))
            throw new Error("Playlist mode should be one of " + enums.PlaylistMode.toString())
        this._playlist.mode = enums.PlaylistMode.indexOf(mode);
    }
